...
function registerIPC() {
  ...
  ipcMain.handle('write-reshade-config', async (_, ffxiPath, effects) => {
    try {
      ...
      // Write the preset file with effect toggles and values
      const presetLines = [
        '; ReShadePreset.ini — generated by XI Launcher',
        '',
        '[LumaSharpen.fx]',
      ...
      // Also update the Effects= line in ReShade.ini to enable/disable shaders
      if (fs.existsSync(iniPath)) {
        ...
        const effectsLine = `Effects=${enabledEffects.join(',')}`;

        if (ini.match(/^Effects=.*/m)) {
          ini = ini.replace(/^Effects=.*/m, effectsLine);
        } else {
          // Insert after PreprocessorDefinitions or at end of [GENERAL]
          ini = ini.replace(/(\[GENERAL\][^\[]*)/s, `$1${effectsLine}\r\n`);
        }
  ...
  ipcMain.handle('read-reshade-config', async (_, ffxiPath) => {
    try {
      ...
      // Read enabled effects from main INI, values from preset file
      const iniSections = parseIni(path.join(dllDir, 'ReShade.ini'));
      const presetSections = parseIni(path.join(dllDir, 'ReShadePreset.ini'));

      const enabledList = (iniSections['GENERAL']?.Effects || '').split(',').map(s => s.trim()).filter(Boolean);
